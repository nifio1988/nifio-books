...
- `isBookUpcoming(book)` — the **single source of truth** for whether a book counts as "in uscita": `prossimamente === true` AND no real `amazon` link. A valid Amazon link always wins over a stale `prossimamente: true` left in `books.json`. Every PROSSIMAMENTE badge in the site (grid card, book modal, release modal) must go through this function — don't reintroduce a raw `book.prossimamente === true` check anywhere, that was the exact bug fixed on 2026-08-11 (the release modal used to hardcode the badge/content instead of reading `books.json` at all).
- `openBookModal()` — populates the book detail modal; hides the Amazon button when `amazon` is empty and the "Sinossi" label/paragraph when `sinossi` is empty, hides/shows the "Prossimamente" badge via `isBookUpcoming()`.
...
- `renderReleaseSpotlight()` — the "Libri in uscita" modal (`#releaseOverlay`) is **data-driven**, not hardcoded: it looks up `FEATURED_RELEASE_TITLE` in `books.json` (currently "GIOVINAZZO Through My Eyes - Full Color Edition") and renders its real cover/title/description. The badge and the whole countdown block (`#releaseCountdownSection`) only show when `isBookUpcoming()` is true for that book; otherwise the modal shows an "available now" tagline plus a real Amazon button instead. It auto-opens on load same as before, gated by two ready flags (`pageLoaded`, `releaseSpotlightReady`) so it can't fire before `books.json` has loaded.
- `updateCountdown()` — ticks down to a hardcoded release date constant (`RELEASE_TARGET`, currently 2026-07-24) only while the release spotlight section is visible (i.e. only when the featured book is actually upcoming). When the target has passed, it swaps the 4 digit tiles for a `#countdownExpired` message instead of showing zeros/negatives — don't "fix" this by assuming the tiles should always render, and don't decouple it from `isBookUpcoming()` again.
